...
def calculate_bert_score(
    ...
    lang: str = "en") -> float:
    """Compute Bleu score."""
    flattened_targets = [' '.join(target) for target in targets]

    P, R, F1 = bert_score.score(predictions, flattened_targets, lang=lang)

    avg_bertscore = float(F1.mean()) * 100.0
    individual_scores = [float(f) * 100.0 for f in F1]

    return avg_bertscore, individual_scores
...
def doc_evaluate(
    ...
    assert metric in ['ExactAccuracy', 'RelaxedAccuracy', 'ANLS', 'ContainAccuracy', 
                        'CIDEr', 'BLEU1', 'BLEU2', 'BLEU3', 'BLEU4', 'RougeL', 'Meteor', 'Bertscore']
    if metric=='ExactAccuracy': # case sensitive
        ...
        score, scores = calculate_bert_score(targets, predictions)
    return score, scores 
...
if __name__ == '__main__':
    ...
    for metric in ['BLEU1', 'BLEU2', 'BLEU3', 'BLEU4', 'RougeL', 'Meteor', 'CIDEr', 'Bertscore']:
        score, scores = doc_evaluate(metric=metric, targets=targets, predictions=predictions)
        ic(metric, score, scores)
